...
public class Q1 : MonoBehaviour
{
    ...
    public void OnGenerateBtnClick()
    {
        if (xInputField != null && yInputField != null && zInputField != null)
        {
            ...
            vData.x = float.Parse(xInputField.text);
            vData.y = float.Parse(yInputField.text);
            vData.z = float.Parse(zInputField.text);
            Q1MyGrid.vectorData = vData;
        }
        if(gridRootNode!=null)
        {
            Q1MyGrid myGrid=gridRootNode.GetComponent<Q1MyGrid>();
            if(myGrid!=null)
            {
                myGrid.gridItemPrefab=gridItemPrefab;
                myGrid.GenerateGrid();
            }
        }
    }
